...
if ($PluginDir -ne $null) {
    if (Test-Path "$PluginDir\Resources\SignProjectForAntiCheat.ps1") {
        & "$PluginDir\Resources\SignProjectForAntiCheat.ps1" -EngineDir "$EngineDir" -ProjectDir "$ProjectDir" -PluginDir "$PluginDir" -StageDir "$StageDirectory"
        exit $LASTEXITCODE
    }
}
