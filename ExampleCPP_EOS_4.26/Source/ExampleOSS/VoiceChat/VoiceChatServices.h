...
public:
	...
	void BP_GetJoinedChannels(
		const TArray<FString>& JoinedChannels);

	UFUNCTION(BlueprintCallable, Category = "Voice")
	TArray<FString> GetPlayersInChannel(
		const FString& ChannelName);
	
	UFUNCTION(BlueprintImplementableEvent, Category = "Voice", meta = (DisplayName="GetPlayersInChannel"))
	void BP_GetPlayersInChannel(
	...
	void SetTransmitMode(
		const enum EVoiceChatTransmitModeCPP& TransmitMode);
	...
	void HandleVoiceChatChannelJoined(
		const FString& ChannelName);

	void HandleVoiceChatChannelExited(
		const FString& ChannelName,
		const struct FVoiceChatResult& Result);
	
	void HandleVoiceChatPlayerAdded(
		const FString& ChannelName,
		const FString& PlayerName);
	
	void HandleVoiceChatPlayerRemoved(
		const FString& ChannelName,
		const FString& PlayerName);
	
	void HandleVoiceChatPlayerMuteUpdated(
		const FString& ChannelName,
		const FString& PlayerName,
		const bool bIsMuted);
